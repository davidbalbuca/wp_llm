...
type PendingWait struct {
	...
	// EsperandoRespuesta marca que se le ofrecio el menu y todavia no contesta. La busqueda se
	// consulta cada pocos segundos y se queda en SIN_CONDUCTOR mientras el cliente no responde:
	// sin esta marca se le mandaria el mismo menu una y otra vez. Se limpia en cuanto contesta,
	// para que la SIGUIENTE ronda pueda volver a preguntarle.
	//
	// Antes se llamaba PreguntoEspera y no se limpiaba nunca: por eso a Carlos (23/09) se le
	// pregunto una sola vez y, cuando esa espera vencio, se le despidio sin volver a consultarle.
	EsperandoRespuesta bool `json:"esperando_respuesta,omitempty"`
	// RondaEspera es cuantas veces ya se le ofrecio esperar. Decide QUE se le ofrece: en las
